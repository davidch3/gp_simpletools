...
# Logging all the connections which were idle for more than X hours
  psql -t -ac "select usename,procpid,current_query,query_start,backend_start,xact_start from pg_stat_activity where current_query='<IDLE>' and now()-backend_start>interval '${INT_IDLE_CONN} hours'" postgres >> /home/gpadmin/gpAdminLogs/kill_long_idle_${LOGDATE}.log
}
...
# Terminate pid's of IDLE CONNECTIONS for more than X hours :
  psql -A -t -c "SELECT 'select pg_terminate_backend('||procpid||');select pg_sleep(1);' from pg_stat_activity where current_query = '<IDLE>' and now()-backend_start>interval '${INT_IDLE_CONN} hours'" postgres | psql -a postgres >> /home/gpadmin/gpAdminLogs/kill_long_idle_${LOGDATE}.log
}
