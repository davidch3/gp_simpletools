...
def run_psql_utility(v_sql):
    psql_cmd = """PGOPTIONS='-c gp_role=utility' psql -v ON_ERROR_STOP=1 -d %s -p %s -A -X -t -c \"%s\" """ % (dbname, portno, v_sql)
    #plpy.info(psql_cmd)
...
cmd1 = rows[0]["cmd1"]
rows = plpy.execute("select string_agg('export LANG=en_US.utf8;ls -l --time-style=''+%Y-%m-%d_%H:%M:%S'' ' ||pg_tablespace_location(oid)||'/'||current_setting('gp_dbid')||'/*/'||(SELECT oid from pg_database where datname=current_database())||' >> /tmp/fs_'||current_setting('gp_dbid')||'.dat',' ; ') as cmd2 from pg_tablespace where oid not in (1663,1664);")
cmd2 = rows[0]["cmd2"]
if cmd2 is None:
    ls_cmd = cmd1
else:
...
sed_cmd = "sed -i 's/[ ]\{1,\}/|/g;/?/d;/^total/d;/^总用量/d;/^\//d;/^$/d' "+filename
#plpy.info(sed_cmd)
...
copy_sql = "copy public.gp_seg_size_ora from '"+filename+"' delimiter '|' SEGMENT REJECT LIMIT 10 ROWS;"
